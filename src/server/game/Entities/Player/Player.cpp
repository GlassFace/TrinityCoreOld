...
void Player::AutoQuestCompleteDisplayQuestGiver(uint32 p_questId){
	if (sWorld->getIntConfig(CONFIG_AUTOCOMPLETE_DELAY) == 0) return;
	...
	TempSummon *_sum = SummonCreature((*result)[0].GetUInt32(), GetPositionX(), GetPositionY(), GetPositionZ(), 3.3f, TEMPSUMMON_TIMED_DESPAWN, sWorld->getIntConfig(CONFIG_AUTOCOMPLETE_DELAY) * 1000);
