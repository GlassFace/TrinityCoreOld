...
bool Player::CanDropQuestItem(uint32 itemid) // LASYAN: return true if at least one quest can be done by the player
{
	...
	TC_LOG_DEBUG("lasyan", "START CanDropQuestItem for item %d [%s]", itemid, sObjectMgr->GetItemTemplate(itemid)->Name1.c_str());
	...
	for (ObjectMgr::QuestMap::const_iterator iter = _allQuests.begin(); iter != _allQuests.end(); ++iter)
	{
		...
		for (uint8 i = 0; i < QUEST_ITEM_OBJECTIVES_COUNT; i++)
		{
			...
			if (qInfo->RequiredItemCount[i] != 0 && GetItemCount(itemid, false) < qInfo->RequiredItemCount[i])
			{
				...
				TC_LOG_DEBUG("lasyan", "END CanDropQuestItem Item can be dropped for the quest --> TRUE");
				return true;
			}
			else
			{
				TC_LOG_DEBUG("lasyan", " |- Item already at max");
				continue;
			}
		}
	}
	TC_LOG_DEBUG("lasyan", "END CanDropQuestItem No quest is satisfying --> FALSE");
	return false;
}
...
{
	ItemTemplate const * it = sObjectMgr->GetItemTemplate(itemid);
	TC_LOG_DEBUG("lasyan", "START GetAvailableQuestsForItem for item %d [%s]", itemid, it->Name1.c_str());
	...
	{
		TC_LOG_DEBUG("lasyan", " |- No quest found for item --> FALSE");
		return _allQuests;
	}
	...
	do
	{
		...
		TC_LOG_DEBUG("lasyan", " |- Do quest %d [%s]", questid, sObjectMgr->GetQuestTemplate(questid)->GetTitle().c_str());

		QuestStatus status = GetQuestStatus(questid);
		if (status != QUEST_STATUS_NONE)
		{
			TC_LOG_DEBUG("lasyan", " |- Quest has status %d", status);
			continue;
		}
		...
		if (!qInfo->HasSpecialFlag(QUEST_SPECIAL_FLAGS_DELIVER))
		{
			TC_LOG_DEBUG("lasyan", " |- Quest has not special flag QUEST_SPECIAL_FLAGS_DELIVER");
			continue;
		}
		...
		{
			TC_LOG_DEBUG("lasyan", " |- Player doesn't satisfy quest");
			continue;
		}

		if (!qInfo->IsRepeatable() && getRewardedQuests().find(questid) != getRewardedQuests().end())
		{
			TC_LOG_DEBUG("lasyan", " |- Quest already rewarded");
