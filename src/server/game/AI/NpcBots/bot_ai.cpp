...
void bot_minion_ai::SetStats(bool force, bool shapeshift)
{
    ...
    if (force)
    {
		...
		// LASYAN : Reset Equipment
		master->InitBotEquips(me);
    }
...
bool bot_ai::CanBotAttack(Unit* target, int8 byspell) const
{
	...
	if (m_attackFromCmd) return true;
    return
       (target->IsAlive() &&
       target->IsVisible() &&
...
Unit* bot_ai::_getTarget(bool byspell, bool ranged, bool &reset) const
{
    ...
    uint8 followdist = master->GetBotFollowDist();
    float foldist = _getAttackDistance(float(followdist));
	if (!u && master->IsAlive() && (me->GetDistance(master) > foldist || (mytar && master->GetDistance(mytar) > foldist && me->GetDistance(master) > foldist)) && !m_attackFromCmd)
    {
...
void bot_minion_ai::GetNearbyPotentialTargetsList(std::list<Unit*> &targets, float maxdist, float mindist, bool forCC) const
{
	CellCoord p(Trinity::ComputeCellCoord(me->GetPositionX(), me->GetPositionY()));
	Cell cell(p);
	cell.SetNoCreate();

	NearbyPotentialHostileUnitCheck check(me, maxdist, mindist, this, forCC);
	Trinity::UnitListSearcher <NearbyPotentialHostileUnitCheck> searcher(me, targets, check);

	TypeContainerVisitor<Trinity::UnitListSearcher <NearbyPotentialHostileUnitCheck>, WorldTypeMapContainer > world_unit_searcher(searcher);
	TypeContainerVisitor<Trinity::UnitListSearcher <NearbyPotentialHostileUnitCheck>, GridTypeMapContainer >  grid_unit_searcher(searcher);
...
void bot_minion_ai::OnOwnerDamagedBy(Unit* attacker)
{
	...
	if (!attacker->IsWithinDist(me, maxdist) && !m_attackFromCmd)
        return;
