        ...
        void doDefend(uint32 diff)
        {
            ...
            if (IsSpellReady(PSYCHIC_SCREAM_1, diff, false))
            {
                if (!m_attackers.empty() && (!IsTank(master) || GetHealthPCT(master) < 75))
                {
					...
					//LASYAN : if others creatures  all around, don't send the spell!
					std::list<Unit*> targets;
					GetNearbyPotentialTargetsList(targets, SIGHT_RANGE_UNIT);
					if (tCount > 1 && targets.size() == 0 && doCast(me, GetSpell(PSYCHIC_SCREAM_1)))
                    {
                ...
                if (!b_attackers.empty())
                {
					...
					std::list<Unit*> targets;
					GetNearbyPotentialTargetsList(targets, SIGHT_RANGE_UNIT);
					if (tCount > 0 && targets.size() == 0 && doCast(me, GetSpell(PSYCHIC_SCREAM_1)))
                    {
                        SetSpellCooldown(PSYCHIC_SCREAM_1, 22000);
                        return;
                    }
