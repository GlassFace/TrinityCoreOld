...
class NearbyPotentialHostileUnitCheck
{
public:
	explicit NearbyPotentialHostileUnitCheck(Unit const* unit, float maxdist, float mindist, bot_ai const* m_ai, bool forCC) :
		me(unit), max_range(maxdist), min_range(mindist), ai(m_ai), m_forCC(forCC) { }
	bool operator()(Unit* u)
	{
		if (!PvP && (u->ToPlayer() || (u->ToCreature() && u->ToCreature()->GetBotAI())))
...
private:
	...
	bot_ai const* ai;
	bool m_forCC;
	NearbyPotentialHostileUnitCheck(NearbyPotentialHostileUnitCheck const&);
};
